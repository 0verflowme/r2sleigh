...
mod tests {
    ...
    use r2ssa::{
        CanonicalInstructionSite, CanonicalStorageId, CanonicalStorageSpace,
        SourceCallArgumentSpec, SourceCallSiteInterface, SsaArtifact,
    };
    ...
    fn accounting(
        ...
        let target = Varnode::ram(target_addr, 8);
        ...
        let mut arch = ArchSpec::new("certified-call-region-test");
        ...
        CertifiedSingleBlockAccounting::from_projection_block(&certified, 0x7500)
            .expect("direct-call accounting")
    }

    fn region() -> CertifiedDirectCallBlockRegion {
        CertifiedDirectCallBlockRegion::from_accounting(accounting(
            0x7600,
            b"direct-call-region-revision-1",
            "test-call-abi",
        ))
        .expect("direct-call region")
    }

    #[test]
    fn exact_void_direct_call_exposes_only_open_boundary_identity() {
        let region = region();
        let report = region.audit();

        assert!(report.has_exact_direct_call(), "{report:?}");
        assert!(!region.has_remaining_obligation_residuals());
        assert_eq!(region.open_callee_target(), 0x7600);
        assert_eq!(region.open_fallthrough_successor(), 0x7504);
        assert_eq!(region.raw_identity().block_addr(), 0x7500);
        assert_eq!(region.raw_identity().op_index(), 1);
        assert_eq!(
            region.interface_revision(),
            b"direct-call-region-revision-1"
        );
        assert_eq!(region.calling_convention(), "test-call-abi");
        assert_eq!(region.witness().arguments().len(), 1);
        assert_eq!(region.call().arguments().len(), 1);
        assert!(region.body().accounting().memory_statements().is_empty());
        assert!(region.body().audit().requires_control_region());
    }

    #[test]
    fn schema_and_producer_mutations_fail_audit() {
        let region = region();
        let mut corrupted = region.clone();
        corrupted.schema_version += 1;
        assert!(!corrupted.audit().has_exact_direct_call());

        let mut corrupted = region;
        corrupted.call_producer = CanonicalInstructionId {
            block_addr: 0x7500,
    ...
    #[test]
    fn foreign_semantic_call_fails_audit() {
        let region = region();
        let foreign = CertifiedDirectCallBlockRegion::from_accounting(accounting(
            0x7610,
            b"foreign-direct-call-revision",
            "foreign-call-abi",
        ))
        .expect("foreign direct-call region");
        let mut corrupted = region;
        corrupted.semantic_call = foreign.semantic_call;

        assert!(!corrupted.audit().has_exact_direct_call());
    }

    #[test]
    fn deleted_or_duplicate_mapping_fails_audit() {
        let region = region();
        let call_mapping = region
            .mappings
            .iter()
            .find(|mapping| {
                matches!(
        ...
        let mut deleted = region.clone();
        deleted.mappings = deleted
            .mappings
            .iter()
            .filter(|mapping| mapping.obligation() != call_mapping.obligation())
            .cloned()
            .collect::<Vec<_>>()
            .into_boxed_slice();
        assert!(!deleted.audit().has_exact_direct_call());

        let mut duplicated = region;
        let mut mappings = duplicated.mappings.to_vec();
        mappings.push(call_mapping);
        duplicated.mappings = mappings.into_boxed_slice();
        assert!(!duplicated.audit().has_exact_direct_call());
    }
}
