...
fn dereference_rendered_address(expr: CExpr, unwrap_direct_address: bool) -> CExpr {
    match expr {
        CExpr::Observed { id, expr } => CExpr::observed(
            id,
            dereference_rendered_address(*expr, unwrap_direct_address),
        ),
        CExpr::AddrOf(inner) if unwrap_direct_address => *inner,
...
impl<'a> FoldingContext<'a> {
    ...
    fn normalize_prepared_call_arg_expr(&self, arg: CExpr) -> CExpr {
        let original_param_home = match arg.unobserved() {
        ...
        if let Some(name) = original_param_home
            && matches!(normalized.unobserved(), CExpr::Deref(inner) if matches!(inner.unobserved(), CExpr::Var(inner_name) if self.spelling(*inner_name).eq_ignore_ascii_case(&*self.spelling(name))))
        {
            return arg;
        }
        normalized
    ...
    fn call_arg_requires_result_rebuild(&self, expr: &CExpr) -> bool {
        let CExpr::Call { args, .. } = expr.unobserved() else {
            return false;
        };
        match self.source_proof_for_call_expr(expr.unobserved()) {
        ...
        args.iter().any(|arg| {
                self.call_arg_contains_transient_name(arg, 0)
                || self.call_arg_contains_stack_placeholder(arg, 0)
                || matches!(arg.unobserved(), CExpr::Call { .. })
                || self.expr_is_generic_entry_arg_like(arg)
        })
    }
    ...
    pub(super) fn recover_call_arg_expr_from_source_var(
        ...
        let raw_expr = self
            .definition_for_value_id(source_value_id)
            .cloned()
            .map(|raw| {
        ...
        let visible_expr = self
            .use_info()
            .render_definition_for_value(source_value_id)
            .cloned()
            .map(|visible_def| {
                let mut imported_visited = HashSet::new();
        ...
        if best_is_low_signal {
            ...
            best = prefer(
                best,
                self.use_info()
                    .render_semantic_value_for_value(source_value_id)
        ...
        if matches!(best.unobserved(), CExpr::Var(name) if self.is_autogenerated_stack_home_name(&*self.spelling(*name)))
            && let Some(raw_best) = self.definition_for_value_id(source_value_id).cloned()
    ...
    fn stable_result_call_arg_owner_expr_for_source(
        ...
        if let CExpr::Var(name) = &owner {
            ...
            if is_generic_arg_name(&*self.spelling(*name))
    ...
    fn param_home_alias_expr_for_call_arg_binding(
        ...
        match &binding.arg {
            ...
            analysis::SemanticCallArg::FallbackExpr(CExpr::Var(name)) => {
                if let Some(slot) = self.stack_slot_provenance_for_name(&*self.spelling(*name)) {
                    offsets.insert(slot.offset);
                }
                if let Some(prov) = self.forwarded_value_for_name(&*self.spelling(*name))
                    && let Some(offset) = prov.stack_slot
                {
                    offsets.insert(offset);
                }
    ...
    fn render_imported_semantic_arg_value(
            ...
            && matches!(addr.base, analysis::BaseRef::Value(_))
        {
            if let analysis::BaseRef::Value(base_value) = &addr.base
                ...
                && !matches!(expr.unobserved(), CExpr::StringLit(_))
            {
                return Some(expr);
            }
            let mut visited = HashSet::new();
    ...
    pub(super) fn prepared_imported_semantic_arg_expr(
        ...
        let owner_for_var = |var: &r2ssa::SSAVar| {
            let preferred = prepared
                ...
                .cloned()
                .filter(|expr| {
                    preserve_pointer_identity
                        || !matches!(expr.unobserved(), CExpr::AddrOf(_))
                })
        ...
        match value {
            ...
            analysis::SemanticValue::Scalar(analysis::ScalarValue::Expr(expr)) => {
                ...
                let should_try_prepared_owner_rewrite = false;
    ...
    fn render_stable_stack_scalar_expr(&self, offset: i64) -> Option<CExpr> {
        ...
        let best = self
            ...
            .unwrap_or(rewritten);
        (!matches!(best.unobserved(), CExpr::AddrOf(_) | CExpr::Deref(_))).then_some(best)
    }
