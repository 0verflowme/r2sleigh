...
//! and bounds. It is not a proof, a `CertifiedC` value, or execution authority
...
/// Execute one closed, phi-free prepared-SSA function from an exact set of
/// artifact-local boundary overrides.  The remaining undefined machine inputs
/// are initialized deterministically, and a concrete stack domain is supplied
/// for the standard frame save/restore envelope.
///
/// This is crate-private because admitting a semantic-C candidate still belongs
/// to a sealed function-specific certificate.  It exists only so those sealed
/// differential harnesses execute the retained prepared SSA instead of
/// reconstructing a mathematical source program from the certificate.
pub(crate) fn execute_prepared_single_block_return(
    artifact: &SsaArtifact,
    overrides: impl IntoIterator<Item = (ValueId, DifferentialBitVector)>,
    max_source_steps: u32,
) -> Result<Box<[DifferentialBitVector]>, String> {
    ...
    let [block_addr] = function.block_addrs() else {
        return Err("prepared source is not one closed block".to_string());
    };
    let block = function
        .get_block(*block_addr)
        .ok_or_else(|| "prepared source entry block is missing".to_string())?;
    if function.entry != *block_addr
        || !block.phis.is_empty()
        || !function.predecessors(*block_addr).is_empty()
        || !function.successors(*block_addr).is_empty()
    {
        return Err("prepared source is not one closed phi-free block".to_string());
    }

    let mut initial = DifferentialState::for_artifact(artifact)?;
    for graph_value in &artifact.graph().values {
        if artifact.graph().def_inst(graph_value.id).is_some()
            || graph_value.var.constant_bits().is_some()
        {
            continue;
        }
        let width_bits = graph_value
            .var
            .size
            .checked_mul(8)
            .ok_or_else(|| "prepared source input width overflow".to_string())?;
        ...
        let bitvector = DifferentialBitVector::new(width_bits, bits)
            .ok_or_else(|| format!("unsupported prepared source input width {width_bits}"))?;
        initial.set_value(graph_value.id, bitvector);
    }
    for (value, bitvector) in overrides {
        let graph_value = artifact
            .graph()
            .value(value)
            .ok_or_else(|| format!("prepared source override {value:?} is foreign"))?;
        if graph_value.var.size.checked_mul(8) != Some(bitvector.width_bits())
            || artifact.graph().def_inst(value).is_some()
        {
            return Err(format!(
                "prepared source override {value:?} has the wrong width or is not an input"
            ));
        }
        initial.set_value(value, bitvector);
        let Some(override_storage) = graph_value
            .canonical_storage
            .filter(|storage| storage.space == CanonicalStorageSpace::Register)
        else {
            continue;
        };
        let override_end = override_storage
            .offset
            .checked_add(u64::from(override_storage.size))
            .ok_or_else(|| "prepared source override register range overflow".to_string())?;
        let aliases = artifact
            ...
            .filter(|candidate| {
                candidate.id != value
                    && artifact.graph().def_inst(candidate.id).is_none()
                    && candidate.var.constant_bits().is_none()
            })
            .filter_map(|candidate| {
                candidate
                    .canonical_storage
                    .filter(|storage| storage.space == CanonicalStorageSpace::Register)
                    .map(|storage| (candidate.id, storage))
            })
            .collect::<Vec<_>>();
        for (alias, alias_storage) in aliases {
            let alias_end = alias_storage
                .offset
                .checked_add(u64::from(alias_storage.size))
                .ok_or_else(|| "prepared source alias register range overflow".to_string())?;
            let overlap_start = override_storage.offset.max(alias_storage.offset);
            let overlap_end = override_end.min(alias_end);
            if overlap_start >= overlap_end {
                continue;
            }
            let overlap_width = u32::try_from((overlap_end - overlap_start) * 8)
                .map_err(|_| "prepared source alias width overflow".to_string())?;
            let override_shift = u32::try_from((overlap_start - override_storage.offset) * 8)
                .map_err(|_| "prepared source override shift overflow".to_string())?;
            let alias_shift = u32::try_from((overlap_start - alias_storage.offset) * 8)
                .map_err(|_| "prepared source alias shift overflow".to_string())?;
            let alias_width = alias_storage
                .size
                .checked_mul(8)
                .ok_or_else(|| "prepared source alias width overflow".to_string())?;
            let current = initial
                .values()
                .get(&alias)
                .copied()
                .ok_or_else(|| "prepared source alias input was not initialized".to_string())?;
            let mask = source_width_mask(overlap_width) << alias_shift;
            let overlay = ((bitvector.bits() >> override_shift) << alias_shift) & mask;
            initial.set_value(
                alias,
                DifferentialBitVector::new(alias_width, (current.bits() & !mask) | overlay)
                    .ok_or_else(|| "prepared source alias width is unsupported".to_string())?,
            );
        }
    }

    let source_memory_space = block.ops.iter().enumerate().find_map(|(op_index, op)| {
    ...
    if let Some(source_memory_space) = source_memory_space {
        let memory_space = MachineAddressSpace::from(source_memory_space);
        for address in STACK_BASE - 8..STACK_BASE + 8 {
            initial.set_memory_byte(
    ...
    let mut run = execute_source_inner(artifact, *block_addr, &mut state, limits)
        .map_err(|failure| format!("prepared source execution failed: {failure:?}"))?;
    let values = source_return_values(artifact, *block_addr, &run)
        .map_err(|failure| format!("prepared source return failed: {failure:?}"))?;
    run.outcome = DifferentialBoundaryOutcome::Returned {
        values: values.into_boxed_slice(),
    };
    let DifferentialBoundaryOutcome::Returned { values } = run.outcome else {
        unreachable!("prepared source outcome was just terminalized")
    };
    Ok(values)
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct PreparedFunctionLimits {
    pub max_block_steps: u32,
    pub max_instruction_steps: u32,
    pub max_memory_bytes: u32,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct PreparedFunctionExecution {
    pub returned: Box<[DifferentialBitVector]>,
    pub memory_events: Box<[DifferentialMemoryEvent]>,
    pub block_steps: u32,
    pub instruction_steps: u32,
}

/// Execute a closed prepared SSA/CFG with exact artifact-local inputs and a
/// finite byte-memory domain. Loop phis are selected by the concrete incoming
/// edge and rebound on each dynamic visit. Every block, phi, and operation is
/// charged against an explicit budget.
///
/// This remains crate-private: function-specific sealed differentials decide
/// which prepared artifacts, inputs, memory objects, and typed candidates are
/// admissible.
pub(crate) fn execute_prepared_function_return(
    artifact: &SsaArtifact,
    overrides: impl IntoIterator<Item = (ValueId, DifferentialBitVector)>,
    initial_memory: impl IntoIterator<Item = (DifferentialMemoryLocation, u8)>,
    limits: PreparedFunctionLimits,
) -> Result<PreparedFunctionExecution, String> {
    if limits.max_block_steps == 0
        || limits.max_instruction_steps == 0
        || limits.max_memory_bytes == 0
    {
        return Err("prepared function limits must all be nonzero".to_string());
    }
    let mut initial = DifferentialState::for_artifact(artifact)?;
    for graph_value in &artifact.graph().values {
        if artifact.graph().def_inst(graph_value.id).is_some()
        ...
        let width_bits = graph_value
            ...
            .size
            .checked_mul(8)
            .ok_or_else(|| "prepared function input width overflow".to_string())?;
        initial.set_value(
            graph_value.id,
            DifferentialBitVector::new(width_bits, 0)
                .ok_or_else(|| format!("unsupported prepared function input width {width_bits}"))?,
        );
    }
    for (value, bitvector) in overrides {
        let graph_value = artifact
            .graph()
            .value(value)
            .ok_or_else(|| format!("prepared function override {value:?} is foreign"))?;
        if graph_value.var.size.checked_mul(8) != Some(bitvector.width_bits())
            || artifact.graph().def_inst(value).is_some()
        {
            return Err(format!(
                "prepared function override {value:?} has the wrong width or is not an input"
            ));
        }
        initial.set_value(value, bitvector);
    }
    for (location, byte) in initial_memory {
        initial.set_memory_byte(location, byte);
        if initial.memory().len() > limits.max_memory_bytes as usize {
            return Err("prepared function memory budget exceeded".to_string());
        }
    }

    let mut state = ExecutionState::from(&initial);
    let mut predecessor = None;
    let mut block_addr = artifact.function().entry;
    let mut block_steps = 0_u32;
    let mut instruction_steps = 0_u32;
    loop {
        block_steps = block_steps
            .checked_add(1)
            .ok_or_else(|| "prepared function block budget overflow".to_string())?;
        if block_steps > limits.max_block_steps {
            return Err("prepared function block budget exceeded".to_string());
        }
        let graph = artifact.graph();
        let block = graph
            .block_id_for_addr(block_addr)
            .and_then(|id| graph.block(id))
            .ok_or_else(|| format!("prepared function graph block 0x{block_addr:x} is missing"))?;
        let cfg_block = artifact
            .function()
            .cfg()
            .get_block(block_addr)
            .ok_or_else(|| format!("prepared function CFG block 0x{block_addr:x} is missing"))?;
        let block_cost = u32::try_from(block.insts.len())
            .map_err(|_| "prepared function instruction count overflow".to_string())?;
        instruction_steps = instruction_steps
            .checked_add(block_cost)
            .ok_or_else(|| "prepared function instruction budget overflow".to_string())?;
        if instruction_steps > limits.max_instruction_steps {
            return Err("prepared function instruction budget exceeded".to_string());
        }

        let predecessor_id = predecessor
            .map(|address| {
                graph.block_id_for_addr(address).ok_or_else(|| {
                    "prepared function predecessor graph block is missing".to_string()
                })
            })
            .transpose()?;
        ...
        for inst_id in &block.insts {
            let inst = graph
                .inst(*inst_id)
                .ok_or_else(|| format!("prepared function instruction {inst_id:?} is missing"))?;
            let InstPayload::Phi { predecessors } = &inst.payload else {
                continue;
            };
            let incoming = predecessor_id.ok_or_else(|| {
                "prepared function entry or predecessor-free edge reaches a phi".to_string()
            })?;
            let input_index = predecessors
                .iter()
                .position(|candidate| *candidate == incoming)
                .ok_or_else(|| "prepared function phi predecessor is absent".to_string())?;
            let source = inst
                .inputs
                .get(input_index)
                .copied()
                .ok_or_else(|| "prepared function phi input is absent".to_string())?;
            let output = inst
                .output
                .ok_or_else(|| "prepared function phi output is absent".to_string())?;
            let value = source_value(artifact, &state, source)
                .map_err(|failure| format!("prepared function phi failed: {failure:?}"))?;
            phi_values.push((output, value));
        }
        for inst_id in &block.insts {
            if let Some(output) = graph.inst(*inst_id).and_then(|inst| inst.output) {
                state.values.remove(&output);
            }
        }
        for (output, value) in phi_values {
            bind_source_output(artifact, &mut state, output, value)
                .map_err(|failure| format!("prepared function phi bind failed: {failure:?}"))?;
        }
        for inst_id in &block.insts {
            let inst = graph
                .inst(*inst_id)
            ...
            execute_source_inst(artifact, &mut state, *inst_id)
                .map_err(|failure| format!("prepared function operation failed: {failure:?}"))?;
        }
        ...
        let next = match &cfg_block.terminator {
            BlockTerminator::ConditionalBranch {
                ...
                validate_source_direct_target(artifact, block_addr, true).map_err(|failure| {
                    format!("prepared function conditional target failed: {failure:?}")
                })?;
                source_conditional_target(artifact, block_addr, &state).map_err(|failure| {
                    format!("prepared function conditional failed: {failure:?}")
                })?
            }
            BlockTerminator::Branch { target } => {
                validate_source_direct_target(artifact, block_addr, false).map_err(|failure| {
                    format!("prepared function branch target failed: {failure:?}")
                })?;
                *target
            }
            BlockTerminator::Fallthrough { next } => *next,
            BlockTerminator::Return => {
                let returned = source_return_values_from_state(artifact, block_addr, &state)
                    .map_err(|failure| format!("prepared function return failed: {failure:?}"))?;
                return Ok(PreparedFunctionExecution {
            ...
            _ => return Err("prepared function has an unsupported terminator".to_string()),
        };
        if !cfg_block.successors().contains(&next) {
            return Err("prepared function selected a non-successor edge".to_string());
        }
        predecessor = Some(block_addr);
        block_addr = next;
    }
}

fn validate_source_direct_target(
    artifact: &SsaArtifact,
    block_addr: u64,
    conditional: bool,
) -> Result<(), RunFailure> {
    ...
    let inst = graph
        ...
        .ok_or_else(|| RunFailure::Invalid("direct-control instruction is missing".to_string()))?;
    match (&inst.payload, conditional) {
        (InstPayload::Op(SSAOp::CBranch { .. }), true)
        | (InstPayload::Op(SSAOp::Branch { .. }), false) => {}
        _ => {
            return Err(RunFailure::Invalid(
                "direct-control graph terminator differs from its CFG edge".to_string(),
            ));
        }
    }
    Ok(())
...
fn source_return_values_from_state(
    artifact: &SsaArtifact,
    block_addr: u64,
    state: &ExecutionState,
) -> Result<Vec<DifferentialBitVector>, RunFailure> {
    source_return_values_with(artifact, block_addr, |value| {
        source_value(artifact, state, value)
    })
}
...
mod tests {
    ...
    use r2ssa::{
        ...
        SourceCallSiteInterface, SourceFunctionInterface, SourceFunctionReturn,
    };

    fn external_values(artifact: &SsaArtifact) -> Vec<ValueId> {
        artifact
            .graph()
            .values
            .iter()
            .filter(|value| {
                value.var.constant_bits().is_none() && artifact.graph().def_inst(value.id).is_none()
            })
            .map(|value| value.id)
            .collect()
    }
    ...
    fn reverse_le_bool_not_select_fnv_artifact() -> SsaArtifact {
        ...
        let reverse_le = Varnode::unique(0x10, 1);
        let inverted = Varnode::unique(0x18, 1);
        let adjusted = Varnode::unique(0x20, 8);
        let selected = Varnode::unique(0x28, 8);
        let mixed = Varnode::unique(0x30, 8);
        let product = Varnode::unique(0x38, 8);
        let mut block = R2ILBlock::new(0x7060, 4);
        ...
        block.push(R2ILOp::Store {
            space: SpaceId::Ram,
            addr: Varnode::constant(0x900, 8),
            val: product,
        });
        let mut arch = ArchSpec::new("reverse-LE FNV-shaped differential test");
        arch.addr_size = 8;
        arch.set_memory_endianness(Endianness::Little);
        SsaArtifact::raw(&[block], Some(&arch)).expect("reverse-LE FNV-shaped artifact")
    }

    fn select_roots(
        certified: &CertifiedMachineProjection,
        semantic: &SemanticCExpressionLayer,
    ) -> (MachineExprId, SemanticCExprId) {
        let machine = certified.projection();
        let machine_entity = machine
            .entities()
            .iter()
        ...
        let semantic_entity = semantic
            .entity_for_producer(machine_entity.producer())
            .expect("semantic select entity");
        (machine_entity.root(), semantic_entity.root())
    }

    #[test]
    fn reverse_le_boolean_not_select_fnv_chain_matches_exactly() {
        const FNV_OFFSET: u64 = 0xcbf29ce484222325;
        const FNV_PRIME: u64 = 0x100000001b3;
        let artifact = reverse_le_bool_not_select_fnv_artifact();
        let graph = artifact.graph();
        let input = graph
            .inst(graph.blocks[0].insts[0])
            .expect("comparison")
            .inputs[1];
        let product = graph
            .inst(graph.blocks[0].insts[5])
            .expect("product")
            .output
            .expect("product output");
        for input_bits in [0, 25, 26, u64::MAX] {
            let mut state = DifferentialState::for_artifact(&artifact).expect("state");
            state.set_value(
                input,
                DifferentialBitVector::new(64, input_bits).expect("input"),
            );
            for offset in 0..8 {
                state.set_memory_byte(
                    DifferentialMemoryLocation {
                        space: MachineAddressSpace::Ram,
                        byte_address: 0x900 + offset,
                    },
                    0,
                );
            }
            let report =
                check_block_differential(&artifact, 0x7060, &state, DifferentialLimits::default());
            assert_eq!(
                report.conclusion(),
                DifferentialConclusion::NoMismatchObserved,
                "{report:?}"
            );
            let selected = if input_bits < 0x1a {
                input_bits.wrapping_add(0x20)
            } else {
                input_bits
            };
            let expected = (selected ^ FNV_OFFSET).wrapping_mul(FNV_PRIME);
            for run in [
                report.source().expect("source run"),
                report.semantic_c().expect("semantic run"),
            ] {
                assert_eq!(observed_bits(run, product), expected);
            }
        }
    }

    #[test]
    fn recursive_select_audit_rejects_polarity_type_and_memory_arm_corruption() {
        let artifact = reverse_le_bool_not_select_fnv_artifact();
        ...
        let (machine_select, semantic_select) = select_roots(&certified, &semantic);
        assert!(
            audit_semantic_expr_pair(
        ...
        let SemanticCExprKind::Select {
            ...
            if_true,
            if_false,
        } = semantic.expr(semantic_select).expect("select root").kind()
        else {
            unreachable!();
        };
        let (condition, if_true, if_false) = (*condition, *if_true, *if_false);

        let mut swapped = semantic.clone();
        swapped
        ...
        let mut mistyped = semantic.clone();
        mistyped
            .replace_expr_type_for_test(
                condition,
                MachineType::Integer {
                    width_bits: 8,
                    signedness: MachineSignedness::Unsigned,
        ...
        assert_eq!(
            ...
            Err("semantic expression type differs from machine evidence".to_string())
        );

        let mut effectful = semantic;
        effectful
            .replace_expr_kind_for_test(
                if_true,
                SemanticCExprKind::MemoryRead {
        ...
        assert_eq!(
            ...
            Err("semantic expression kind differs from machine evidence".to_string())
        );
    }

    #[test]
    fn recursive_boolean_not_audit_rejects_bitwise_not_corruption() {
        let artifact = reverse_le_bool_not_select_fnv_artifact();
        let certified = CertifiedMachineProjection::from_artifact(&artifact).expect("projection");
        let mut semantic =
            SemanticCExpressionLayer::from_projection(&certified).expect("semantic projection");
        ...
        let machine_entity = machine
            ...
            .expect("machine boolean-not entity");
        let semantic_root = semantic
            .entity_for_producer(machine_entity.producer())
            .expect("semantic boolean-not entity")
            .root();
        let SemanticCExprKind::BooleanNot { input } = semantic
            .expr(semantic_root)
            .expect("boolean-not root")
            .kind()
        else {
            unreachable!();
        };
        ...
        semantic
            .replace_expr_kind_for_test(semantic_root, SemanticCExprKind::BitwiseNot { input })
            .expect("forge bitwise-not");
        assert_eq!(
            audit_semantic_expr_pair(
                machine,
                &semantic,
    ...
    #[test]
    fn direct_call_boundary_matches_without_modeling_the_callee() {
        let artifact = direct_call_artifact();
        let parameter = artifact
            ...
            .value;
        for input in [0, 1, u64::MAX] {
            let mut state = DifferentialState::for_artifact(&artifact).expect("state");
            state.set_value(
                parameter,
                DifferentialBitVector::new(64, input).expect("argument"),
            );
            ...
            assert_eq!(report.admission(), DifferentialCandidateAdmission::Admitted);
            assert!(report.candidate_identity().is_some());
            for run in [report.source(), report.semantic_c()] {
                assert!(matches!(
                    run.map(|run| &run.outcome),
                    Some(DifferentialBoundaryOutcome::OpenDirectCall {
                        target: 0x7600,
                        fallthrough: 0x7504,
                        calling_convention,
                        arguments,
                        ..
                    }) if calling_convention == "callee-test-abi"
                        && arguments.as_ref() == [DifferentialCallArgument {
                            ...
                            value: DifferentialBitVector::new(64, input).expect("expected"),
                        }]
                ));
            }
            if input == 0 {
                let json = serde_json::to_value(&report).expect("direct-call report JSON");
                assert_eq!(json["schema_version"], SEMANTIC_DIFFERENTIAL_SCHEMA_VERSION);
                assert_eq!(
                    json["candidate_identity"]["candidate_kind"],
                    "direct_call_region"
                );
                assert_eq!(json["source"]["outcome"]["kind"], "open_direct_call");
                assert_eq!(json["source"]["outcome"]["target"], "0x0000000000007600");
                assert_eq!(
                    json["source"]["outcome"]["arguments"][0]["value"]["bits_hex"],
                    "0x0000000000000000"
                );
            }
        }
    ...
    #[test]
    fn direct_call_differential_is_separate_and_bounded() {
        let artifact = direct_call_artifact();
        let state = DifferentialState::for_artifact(&artifact).expect("state");
        let missing = check_direct_call_differential(
            &artifact,
            0x7500,
            &state,
            DifferentialLimits::default(),
        );
        assert_eq!(missing.conclusion(), DifferentialConclusion::Incomplete);
        assert!(matches!(
            missing.disposition(),
            DifferentialCaseDisposition::MissingBoundaryInput {
                side: DifferentialSide::Both,
                ..
            }
        ));
        ...
        state.set_value(
            parameter,
            DifferentialBitVector::new(64, 7).expect("argument"),
        );
        ...
        let generic =
            check_block_differential(&artifact, 0x7500, &state, DifferentialLimits::default());
        assert_eq!(
            generic.admission(),
            DifferentialCandidateAdmission::Residual
        );
    ...
    #[test]
    fn direct_call_boundary_mutations_are_observable_mismatches() {
        let artifact = direct_call_artifact();
        let parameter = artifact
            .facts()
            .boundaries
        ...
        state.set_value(
            parameter,
            DifferentialBitVector::new(64, 0x2a).expect("argument"),
        );
        let report = check_direct_call_differential(
            &artifact,
            0x7500,
        ...
        let source = report.source().expect("source run").clone();
        let semantic = report.semantic_c().expect("semantic run").clone();

        let mut mutations = Vec::new();
        let mut wrong_target = semantic.clone();
        if let DifferentialBoundaryOutcome::OpenDirectCall { target, .. } =
            &mut wrong_target.outcome
        {
            *target += 1;
        }
        mutations.push(wrong_target);

        let mut wrong_convention = semantic.clone();
        if let DifferentialBoundaryOutcome::OpenDirectCall {
            calling_convention, ..
        } = &mut wrong_convention.outcome
        {
            calling_convention.push_str("-wrong");
        }
        mutations.push(wrong_convention);

        let mut wrong_argument = semantic.clone();
        if let DifferentialBoundaryOutcome::OpenDirectCall { arguments, .. } =
            &mut wrong_argument.outcome
        {
            arguments[0].value = DifferentialBitVector::new(64, 0x2b).expect("wrong argument");
        }
        mutations.push(wrong_argument);

        let mut deleted_argument = semantic.clone();
        if let DifferentialBoundaryOutcome::OpenDirectCall { arguments, .. } =
            &mut deleted_argument.outcome
        {
            *arguments = Vec::new().into_boxed_slice();
        }
        mutations.push(deleted_argument);

        let mut duplicated_argument = semantic;
        if let DifferentialBoundaryOutcome::OpenDirectCall { arguments, .. } =
            &mut duplicated_argument.outcome
        {
            let duplicate = arguments[0].clone();
            let mut expanded = arguments.to_vec();
            expanded.push(duplicate);
            *arguments = expanded.into_boxed_slice();
        }
        mutations.push(duplicated_argument);

        for mutation in mutations {
            assert!(matches!(
                first_difference(&source, &mutation),
                Some(DifferentialMismatch {
                    kind: DifferentialMismatchKind::BoundaryOutcome,
                    ..
                })
            ));
    ...
    #[test]
    fn terminal_return_matches_at_zero_one_and_wrap_boundaries() {
        ...
        let parameter = artifact
            ...
            .expect("parameter")
            .value;
        for input in [0, 1, u64::MAX - 1, u64::MAX] {
            let mut state = DifferentialState::for_artifact(&artifact).expect("state");
            state.set_value(
                parameter,
                DifferentialBitVector::new(64, input).expect("bitvector"),
            );
            let report = check_terminal_return_differential(
                &artifact,
                &state,
                DifferentialLimits::default(),
            );
            ...
            for run in [report.source(), report.semantic_c()] {
                assert!(matches!(
                    run.map(|run| &run.outcome),
                    Some(DifferentialBoundaryOutcome::Returned { values })
                        if values.as_ref()
                            == [DifferentialBitVector::new(64, expected).expect("expected")]
                ));
            }
        }
    }
    ...
    #[test]
    fn closed_memory_load_return_matches_all_widths_and_endianness() {
        for endianness in [Endianness::Little, Endianness::Big] {
            for width_bytes in [1_u32, 2, 4, 8] {
                let artifact = load_return_artifact(endianness, width_bytes);
                let mut state = DifferentialState::for_artifact(&artifact).expect("state");
                for offset in 0..width_bytes {
                    state.set_memory_byte(
                        DifferentialMemoryLocation {
                            space: MachineAddressSpace::Ram,
                            byte_address: 0x40 + u64::from(offset),
                        },
                        (0x11 * (offset + 1)) as u8,
                    );
                }
                let report = check_memory_terminal_return_differential(
                    &artifact,
                    &state,
                    DifferentialLimits::default(),
                );
                ...
                assert_eq!(
                    report
                        .candidate_identity()
                        .map(DifferentialCandidateIdentity::candidate_kind),
                    Some(DifferentialCandidateKind::MemoryTerminalReturnFunction)
                );
                let expected = (0..width_bytes).fold(0_u64, |bits, offset| {
                    let byte = u64::from((0x11 * (offset + 1)) as u8);
                    match endianness {
                        Endianness::Little => bits | (byte << (offset * 8)),
                        Endianness::Big => (bits << 8) | byte,
                        _ => unreachable!(),
                    }
                });
                for run in [report.source(), report.semantic_c()] {
                    assert!(matches!(
                        ...
                        Some(DifferentialBoundaryOutcome::Returned { values })
                            if values.as_ref()
                                == [DifferentialBitVector::new(width_bytes * 8, expected)
                                    .expect("expected")]
                    ));
                    assert_eq!(run.expect("run").memory_events.len(), 1);
                }
            }
        }
    }
    ...
    #[test]
    fn closed_memory_store_void_preserves_exact_void_boundary() {
        ...
        for run in [report.source(), report.semantic_c()] {
            ...
            assert!(matches!(
                &run.outcome,
                DifferentialBoundaryOutcome::Returned { values } if values.is_empty()
            ));
            assert_eq!(
                run.final_memory
                    .iter()
    ...
    #[test]
    fn closed_memory_ordering_and_aliasing_are_observable() {
        for (endianness, expected_return, expected_bytes) in [
            (Endianness::Little, 0x3040, vec![0x40, 0x30, 0x20, 0x10]),
            (Endianness::Big, 0x1020, vec![0x10, 0x20, 0x30, 0x40]),
        ] {
            let artifact = ordered_memory_return_artifact(endianness);
            let mut state = DifferentialState::for_artifact(&artifact).expect("state");
            for offset in 0..4 {
                state.set_memory_byte(
                    DifferentialMemoryLocation {
            ...
            for run in [report.source(), report.semantic_c()] {
                let run = run.expect("run");
                assert_eq!(run.memory_events.len(), 2);
                assert_eq!(
                    run.memory_events[0].kind,
                    DifferentialMemoryEventKind::Write
                );
                assert_eq!(run.memory_events[1].kind, DifferentialMemoryEventKind::Read);
                assert_eq!(run.memory_events[0].byte_address, 0x40);
                assert_eq!(run.memory_events[1].byte_address, 0x40);
                assert!(matches!(
                    &run.outcome,
                    DifferentialBoundaryOutcome::Returned { values }
                        if values.as_ref()
                            == [DifferentialBitVector::new(16, expected_return).expect("return")]
                ));
                assert_eq!(
                    run.final_memory
                        .iter()
    ...
    #[test]
    fn closed_memory_oob_is_incomplete_on_both_independent_sides() {
        let artifact = load_return_artifact(Endianness::Little, 4);
        let mut state = DifferentialState::for_artifact(&artifact).expect("state");
        for offset in 0..3 {
            state.set_memory_byte(
                DifferentialMemoryLocation {
    ...
    #[test]
    fn closed_memory_charges_access_width_against_memory_budget() {
        let artifact = load_return_artifact(Endianness::Little, 4);
        let state = DifferentialState::for_artifact(&artifact).expect("state");
        let report = check_memory_terminal_return_differential(
            &artifact,
    ...
    #[test]
    fn conditional_return_matches_zero_nonzero_and_boundary_values() {
        let artifact = conditional_return_artifact();
        let parameter = artifact
            .facts()
            .boundaries
            .parameters
        ...
        for input in [0, 1, u64::MAX - 1, u64::MAX] {
            ...
            let report = check_conditional_return_differential(
                &artifact,
                &state,
                DifferentialLimits::default(),
            );
            ...
            assert_eq!(
                report
                    .candidate_identity()
                    .map(DifferentialCandidateIdentity::candidate_kind),
                Some(DifferentialCandidateKind::ConditionalReturnFunction)
            );
            let expected = if input == 0 { 0 } else { u64::MAX };
            for run in [report.source(), report.semantic_c()] {
                assert!(matches!(
                    run.map(|run| &run.outcome),
                    Some(DifferentialBoundaryOutcome::Returned { values })
    ...
    #[test]
    fn conditional_expression_node_budget_is_shared_across_blocks() {
        let artifact = conditional_return_artifact();
        let parameter = artifact
            .facts()
            .boundaries
        ...
        state.set_value(
            parameter,
            DifferentialBitVector::new(64, 1).expect("bitvector"),
        );
        let report = check_conditional_return_differential(
            &artifact,
            &state,
        ...
        assert!(matches!(
            report.disposition(),
            DifferentialCaseDisposition::BudgetExceeded {
                side: DifferentialSide::SemanticC,
            }
        ));
    ...
    #[test]
    fn terminal_return_reports_bind_requested_identity_and_initial_memory_budget() {
        let artifact = terminal_return_artifact();
        let certified = CertifiedMachineProjection::from_artifact(&artifact).expect("certified");
        let requested_identity =
            DifferentialArtifactIdentity::from_origin(certified.origin()).expect("identity");

        let assumptions = r2ssa::AssumptionSet::new(vec![r2ssa::AnalysisAssumption {
            id: Some("foreign-terminal-state".to_string()),
            subject: r2ssa::AssumptionSubject::Target { addr: 0x7070 },
        ...
        let foreign_artifact = artifact.with_assumptions(&assumptions);
        let foreign_state =
            DifferentialState::for_artifact(&foreign_artifact).expect("foreign state");
        let report = check_terminal_return_differential(
            &artifact,
            &foreign_state,
            DifferentialLimits::default(),
        ...
        assert_eq!(report.artifact_identity(), Some(&requested_identity));
        assert_ne!(
            report.artifact_identity(),
            Some(foreign_state.artifact_identity())
        );

        let mut state = DifferentialState::for_artifact(&artifact).expect("state");
        for byte_address in [0x20, 0x21] {
            state.set_memory_byte(
                DifferentialMemoryLocation {
                    space: MachineAddressSpace::Ram,
                    byte_address,
        ...
        assert_eq!(report.conclusion(), DifferentialConclusion::InvalidArtifact);
        assert_eq!(report.artifact_identity(), None);
    }

    #[test]
    fn admitted_integer_operation_families_match_at_sign_and_wrap_boundaries() {
        let a = Varnode::register(0, 1);
        let b = Varnode::register(1, 1);
        let wide = Varnode::register(2, 2);
        let temporary = |offset, size| Varnode::unique(offset, size);
        let mut block = R2ILBlock::new(0x7080, 4);
        block.push(R2ILOp::IntAdd {
            dst: temporary(0x10, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntSub {
            dst: temporary(0x20, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntMult {
            dst: temporary(0x30, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntAnd {
            dst: temporary(0x40, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntOr {
            dst: temporary(0x50, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntXor {
            dst: temporary(0x60, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntNot {
            dst: temporary(0x70, 1),
            src: a.clone(),
        });
        block.push(R2ILOp::IntLeft {
            dst: temporary(0x80, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntRight {
            dst: temporary(0x90, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntSRight {
            dst: temporary(0xa0, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntEqual {
            dst: temporary(0xb0, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntNotEqual {
            dst: temporary(0xc0, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntLess {
            dst: temporary(0xd0, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntSLess {
            dst: temporary(0xe0, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntLessEqual {
            dst: temporary(0xf0, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntSLessEqual {
            dst: temporary(0x100, 1),
            a: a.clone(),
            b: b.clone(),
        });
        block.push(R2ILOp::IntZExt {
            dst: temporary(0x110, 2),
            src: a.clone(),
        });
        block.push(R2ILOp::IntSExt {
            dst: temporary(0x120, 2),
            src: a.clone(),
        });
        block.push(R2ILOp::Trunc {
            dst: temporary(0x130, 1),
            src: wide.clone(),
        });
        block.push(R2ILOp::Subpiece {
            dst: temporary(0x140, 1),
            src: wide,
            offset: 1,
        });
        let output_sizes = [1_u32; 16]
            .into_iter()
            .chain([2, 2, 1, 1])
            .collect::<Vec<_>>();
        for (index, size) in output_sizes.iter().copied().enumerate() {
            block.push(R2ILOp::Store {
                space: SpaceId::Ram,
                addr: Varnode::constant(0x200 + index as u64 * 8, 8),
                val: temporary(0x10 + index as u64 * 0x10, size),
            });
        }
        let mut arch = ArchSpec::new("differential-integer-families");
        arch.addr_size = 8;
        arch.set_memory_endianness(Endianness::Little);
        let artifact = SsaArtifact::raw(&[block], Some(&arch)).expect("integer-family artifact");
        let graph = artifact.graph();
        let add = graph.inst(graph.blocks[0].insts[0]).expect("add graph op");
        let trunc = graph
            .inst(graph.blocks[0].insts[18])
            .expect("trunc graph op");
        let a_id = add.inputs[0];
        let b_id = add.inputs[1];
        let wide_id = trunc.inputs[0];
        let output_ids = graph.blocks[0].insts[..20]
            .iter()
            .map(|inst| graph.inst(*inst).expect("value op").output.expect("output"))
            .collect::<Vec<_>>();
        ...
        for (a_bits, b_bits, wide_bits) in [
            ...
            let mut state = DifferentialState::for_artifact(&artifact).expect("bound state");
            state.set_value(
                a_id,
                DifferentialBitVector::new(8, a_bits).expect("a input"),
            );
            state.set_value(
                b_id,
                DifferentialBitVector::new(8, b_bits).expect("b input"),
            );
            state.set_value(
                wide_id,
                DifferentialBitVector::new(16, wide_bits).expect("wide input"),
            );
            for (index, size) in output_sizes.iter().copied().enumerate() {
                for offset in 0..size {
                    state.set_memory_byte(
                        DifferentialMemoryLocation {
                            space: MachineAddressSpace::Ram,
                            byte_address: 0x200 + index as u64 * 8 + u64::from(offset),
                        },
                        0,
                    );
                }
            ...
            let expected = vec![
                ...
                u64::from(a.wrapping_sub(b)),
                u64::from(a.wrapping_mul(b)),
                u64::from(a & b),
                u64::from(a | b),
                u64::from(a ^ b),
                u64::from(!a),
                u64::from(if shift >= 8 { 0 } else { a << shift }),
                u64::from(if shift >= 8 { 0 } else { a >> shift }),
                u64::from(if shift >= 8 {
                    if a & 0x80 == 0 { 0 } else { 0xff }
                } else {
                    ((a as i8) >> shift) as u8
                }),
                u64::from(a == b),
                u64::from(a != b),
                u64::from(a < b),
                u64::from((a as i8) < (b as i8)),
                u64::from(a <= b),
                u64::from((a as i8) <= (b as i8)),
                u64::from(a),
                u64::from((a as i8 as i16) as u16),
                wide_bits & 0xff,
                (wide_bits >> 8) & 0xff,
            ];
            let report = check_block_differential(
                &artifact,
                0x7080,
                &state,
                DifferentialLimits {
            ...
            for run in [
                report.source().expect("source"),
                report.semantic_c().expect("semantic"),
            ] {
                assert_eq!(run.outputs.len(), output_ids.len());
                assert_eq!(run.memory_events.len(), expected.len());
                for ((value, event), expected) in output_ids
                    .iter()
                    .zip(run.memory_events.iter())
                    .zip(expected.iter())
                {
                    assert_eq!(observed_bits(run, *value), *expected);
                    assert_eq!(event.value.bits(), *expected);
                }
            }
        }
    }
    ...
    #[test]
    fn wrapping_arithmetic_matches_at_all_admitted_widths() {
        let mut block = R2ILBlock::new(0x70c0, 4);
        for (index, size) in [1_u32, 2, 4, 8].into_iter().enumerate() {
            block.push(R2ILOp::IntMult {
                dst: Varnode::unique(0x200 + index as u64 * 0x10, size),
                a: Varnode::register(index as u64 * 0x10, size),
                b: Varnode::constant(3, size),
            });
        }
        for (index, size) in [1_u32, 2, 4, 8].into_iter().enumerate() {
            block.push(R2ILOp::Store {
                space: SpaceId::Ram,
                addr: Varnode::constant(0x300 + index as u64 * 0x10, 8),
                val: Varnode::unique(0x200 + index as u64 * 0x10, size),
            });
        }
        let mut arch = ArchSpec::new("differential-all-widths");
        arch.addr_size = 8;
        arch.set_memory_endianness(Endianness::Little);
        let artifact = SsaArtifact::raw(&[block], Some(&arch)).expect("all-width artifact");
        let input_ids = artifact.graph().blocks[0].insts[..4]
            .iter()
            .map(|inst| artifact.graph().inst(*inst).expect("graph op").inputs[0])
            .collect::<Vec<_>>();
        let output_ids = artifact.graph().blocks[0].insts[..4]
            .iter()
            .map(|inst| {
                artifact
                    .graph()
            ...
            .collect::<Vec<_>>();
        for high_bits in [false, true] {
            let mut state = DifferentialState::for_artifact(&artifact).expect("bound state");
            let mut expected = Vec::new();
            for (index, width) in [8_u32, 16, 32, 64].into_iter().enumerate() {
                let bits = if high_bits {
                    width_mask(width).wrapping_sub(1)
                } else {
                    1
                };
                state.set_value(
                    input_ids[index],
                    DifferentialBitVector::new(width, bits).expect("typed input"),
                );
                expected.push(bits.wrapping_mul(3) & width_mask(width));
                for offset in 0..width / 8 {
                    state.set_memory_byte(
                        DifferentialMemoryLocation {
                            space: MachineAddressSpace::Ram,
                            byte_address: 0x300 + index as u64 * 0x10 + u64::from(offset),
                        },
                        0,
                    );
                }
            ...
            let report =
                check_block_differential(&artifact, 0x70c0, &state, DifferentialLimits::default());
            assert_eq!(
                report.conclusion(),
                DifferentialConclusion::NoMismatchObserved,
                "{report:?}"
            ...
            for run in [
                ...
                assert_eq!(run.memory_events.len(), 4);
                for ((value, event), expected) in output_ids
                    .iter()
                    .zip(run.memory_events.iter())
                    .zip(expected.iter())
    ...
    fn memory_artifact(endianness: Endianness) -> SsaArtifact {
        let address = Varnode::register(0, 8);
        let loaded = Varnode::unique(0x10, 4);
        let mixed = Varnode::unique(0x20, 4);
        ...
        block.push(R2ILOp::IntXor {
            dst: mixed.clone(),
            a: loaded,
            b: Varnode::constant(0x00ff_00ff, 4),
        });
        block.push(R2ILOp::Store {
            space: SpaceId::Ram,
            addr: address,
        ...
        let mut arch = ArchSpec::new("differential-memory-test");
        arch.addr_size = 8;
        arch.set_memory_endianness(endianness);
        SsaArtifact::raw(&[block], Some(&arch)).expect("memory artifact")
    }

    fn observed_bits(run: &DifferentialObservedRun, value: ValueId) -> u64 {
        run.outputs
            .iter()
            .find(|output| output.binding.value() == value)
            .unwrap_or_else(|| panic!("missing observed value {value:?}"))
            .bitvector
            .bits()
    }
    ...
    #[test]
    fn absolute_known_answers_are_non_vacuous() {
        let source = Varnode::register(0, 1);
        let count = Varnode::register(1, 1);
        let add = Varnode::unique(0x10, 1);
        let shifted = Varnode::unique(0x20, 1);
        let compared = Varnode::unique(0x30, 1);
        let wide = Varnode::unique(0x40, 2);
        let extracted = Varnode::unique(0x50, 1);
        let mut block = R2ILBlock::new(0x70e0, 4);
        block.push(R2ILOp::IntAdd {
            dst: add.clone(),
            a: source,
        ...
        block.push(R2ILOp::Store {
            space: SpaceId::Ram,
            addr: Varnode::constant(0x20, 8),
            val: extracted,
        });
        block.push(R2ILOp::Store {
            space: SpaceId::Ram,
            addr: Varnode::constant(0x21, 8),
            val: compared,
        });
        let mut arch = ArchSpec::new("differential-known-answers");
        arch.addr_size = 8;
        arch.set_memory_endianness(Endianness::Little);
        let artifact = SsaArtifact::raw(&[block], Some(&arch)).expect("known-answer artifact");
        let graph = artifact.graph();
        let insts = &graph.blocks[0].insts;
        let source_id = graph.inst(insts[0]).expect("add").inputs[0];
        let count_id = graph.inst(insts[1]).expect("shift").inputs[1];
        let outputs = insts[..5]
            .iter()
            .map(|inst| graph.inst(*inst).expect("value op").output.expect("output"))
            .collect::<Vec<_>>();

        for (source_bits, count_bits, expected) in [
            (0xff, 0x00, [0x00, 0x00, 0x00, 0x0000, 0x00]),
            (0x7f, 0x00, [0x80, 0x80, 0x01, 0xff80, 0xff]),
            (0x80, 0x08, [0x81, 0xff, 0x01, 0xffff, 0xff]),
            (0x7e, 0x08, [0x7f, 0x00, 0x00, 0x0000, 0x00]),
        ] {
            let mut state = DifferentialState::for_artifact(&artifact).expect("bound state");
            state.set_value(
                source_id,
                DifferentialBitVector::new(8, source_bits).expect("source"),
            );
            state.set_value(
                count_id,
                DifferentialBitVector::new(8, count_bits).expect("count"),
            );
            state.set_memory_byte(
                DifferentialMemoryLocation {
                    space: MachineAddressSpace::Ram,
            ...
            let report =
                check_block_differential(&artifact, 0x70e0, &state, DifferentialLimits::default());
            assert_eq!(
                report.conclusion(),
                DifferentialConclusion::NoMismatchObserved,
                "{report:?}"
            ...
            for run in [
                ...
                for (value, expected_bits) in outputs.iter().zip(expected) {
                    assert_eq!(observed_bits(run, *value), expected_bits);
                }
                assert_eq!(run.memory_events.len(), 2);
                assert_eq!(run.memory_events[0].value.bits(), expected[4]);
                assert_eq!(run.memory_events[1].value.bits(), expected[2]);
                assert_eq!(run.final_memory[0].value, expected[4] as u8);
                assert_eq!(run.final_memory[1].value, expected[2] as u8);
            }
        }
    }

    #[test]
    fn deterministic_generated_inputs_match_absolute_results() {
        let a = Varnode::register(0, 1);
        let b = Varnode::register(1, 1);
        let sum = Varnode::unique(0x10, 1);
        let product = Varnode::unique(0x20, 1);
        let mixed = Varnode::unique(0x30, 1);
        let mut block = R2ILBlock::new(0x70e4, 4);
        block.push(R2ILOp::IntAdd {
            dst: sum.clone(),
            a,
            b: b.clone(),
        ...
        let mut arch = ArchSpec::new("differential-generated-inputs");
        arch.addr_size = 8;
        arch.set_memory_endianness(Endianness::Little);
        let artifact = SsaArtifact::raw(&[block], Some(&arch)).expect("generated artifact");
        let a = artifact.graph().insts[0].inputs[0];
        let b = artifact.graph().insts[0].inputs[1];
        let mut base = DifferentialState::for_artifact(&artifact).expect("bound state");
        base.set_memory_byte(
            DifferentialMemoryLocation {
                space: MachineAddressSpace::Ram,
                byte_address: 0x20,
            },
        ...
        for _ in 0..64 {
            ...
            let a_bits = (seed >> 32) as u8;
            seed = seed
                .wrapping_mul(6_364_136_223_846_793_005)
                .wrapping_add(1_442_695_040_888_963_407);
            let b_bits = (seed >> 32) as u8;
            let expected = a_bits.wrapping_add(b_bits).wrapping_mul(3) ^ b_bits;
            let mut state = base.clone();
            state.set_value(
                a,
                DifferentialBitVector::new(8, u64::from(a_bits)).expect("a"),
            );
            state.set_value(
                b,
                DifferentialBitVector::new(8, u64::from(b_bits)).expect("b"),
            );
            let report =
                check_block_differential(&artifact, 0x70e4, &state, DifferentialLimits::default());
            assert_eq!(
                report.conclusion(),
                DifferentialConclusion::NoMismatchObserved,
                "a={a_bits:#x} b={b_bits:#x}: {report:?}"
            );
            for run in [
                report.source().expect("source"),
                report.semantic_c().expect("semantic"),
                ...
                assert_eq!(
                    run.memory_events.last().expect("write").value.bits(),
                    u64::from(expected)
                );
                assert_eq!(run.final_memory[0].value, expected);
            }
        }
    }
    ...
    #[test]
    fn memory_crossing_respects_32_bit_address_domain() {
        let mut block = R2ILBlock::new(0x70f4, 4);
        block.push(R2ILOp::Load {
            dst: Varnode::unique(0x10, 2),
            space: SpaceId::Ram,
            addr: Varnode::register(0, 4),
        });
        let mut arch = ArchSpec::new("differential-32-bit-memory");
        arch.addr_size = 4;
        arch.set_memory_endianness(Endianness::Little);
        let artifact = SsaArtifact::raw(&[block], Some(&arch)).expect("32-bit artifact");
        let address = artifact.graph().insts[0].inputs[0];
        let mut state = DifferentialState::for_artifact(&artifact).expect("bound state");
        state.set_value(
            address,
            DifferentialBitVector::new(32, 0xffff_ffff).expect("address"),
        );
        for (byte_address, byte) in [(0xffff_ffff, 0xaa), (0, 0xbb)] {
            state.set_memory_byte(
                DifferentialMemoryLocation {
                    space: MachineAddressSpace::Ram,
                    byte_address,
        ...
        let report =
            check_block_differential(&artifact, 0x70f4, &state, DifferentialLimits::default());
        assert_eq!(report.conclusion(), DifferentialConclusion::Incomplete);
        assert!(matches!(
            report.disposition(),
            DifferentialCaseDisposition::MemoryOutOfDomain {
                ...
                location: DifferentialMemoryLocation {
                    space: MachineAddressSpace::Ram,
                    byte_address: 0x1_0000_0000,
                },
            }
        ));

        let mut invalid = DifferentialState::for_artifact(&artifact).expect("bound state");
        invalid.set_value(address, DifferentialBitVector::new(32, 0).expect("address"));
        invalid.set_memory_byte(
            DifferentialMemoryLocation {
                space: MachineAddressSpace::Ram,
                byte_address: 0x1_0000_0000,
            },
        ...
        let report =
            check_block_differential(&artifact, 0x70f4, &invalid, DifferentialLimits::default());
        assert_eq!(report.conclusion(), DifferentialConclusion::InvalidInput);

        let mut store_block = R2ILBlock::new(0x70f5, 4);
        store_block.push(R2ILOp::Store {
            space: SpaceId::Ram,
            addr: Varnode::register(0, 4),
            val: Varnode::constant(0x1234, 2),
        });
        let store_artifact =
            SsaArtifact::raw(&[store_block], Some(&arch)).expect("32-bit store artifact");
        let store_address = store_artifact.graph().insts[0].inputs[0];
        let mut store_state =
            DifferentialState::for_artifact(&store_artifact).expect("bound store state");
        store_state.set_value(
            store_address,
            DifferentialBitVector::new(32, 0xffff_ffff).expect("address"),
        );
        for (byte_address, byte) in [(0xffff_ffff, 0xaa), (0, 0xbb)] {
            store_state.set_memory_byte(
                DifferentialMemoryLocation {
                    space: MachineAddressSpace::Ram,
                    byte_address,
                },
        ...
        for trace in [
            report.source_prefix().expect("source prefix"),
            report.semantic_c_prefix().expect("semantic prefix"),
        ] {
            assert!(trace.memory_events.is_empty());
            assert_eq!(
                trace
                    .final_memory
                    .iter()
                    .map(|byte| (byte.location.byte_address, byte.value))
                    .collect::<Vec<_>>(),
                vec![(0, 0xbb), (0xffff_ffff, 0xaa)]
            );
        }
    }

    #[test]
    fn expression_node_budget_is_run_wide() {
        let input = Varnode::register(0, 1);
        let first = Varnode::unique(0x10, 1);
        let second = Varnode::unique(0x20, 1);
        let mut block = R2ILBlock::new(0x70f8, 4);
        block.push(R2ILOp::Copy {
            dst: first.clone(),
            src: input,
        });
        ...
        let mut arch = ArchSpec::new("differential-run-budget");
        arch.addr_size = 8;
        arch.set_memory_endianness(Endianness::Little);
        let artifact = SsaArtifact::raw(&[block], Some(&arch)).expect("budget artifact");
        let input = artifact.graph().insts[0].inputs[0];
        let mut state = DifferentialState::for_artifact(&artifact).expect("bound state");
        state.set_value(input, DifferentialBitVector::new(8, 0x5a).expect("input"));
        state.set_memory_byte(
            DifferentialMemoryLocation {
                space: MachineAddressSpace::Ram,
                byte_address: 0x20,
        ...
        let matched =
            check_block_differential(&artifact, 0x70f8, &state, DifferentialLimits::default());
        assert_eq!(
            matched.conclusion(),
            DifferentialConclusion::NoMismatchObserved
        );
        assert_eq!(
            matched.source().expect("source").final_memory[0].value,
            0x5a
        );
    ...
    #[test]
    fn custom_space_is_isolated_from_ram() {
        let address = Varnode::register(0, 8);
        let mut block = R2ILBlock::new(0x70fc, 4);
        block.push(R2ILOp::Load {
            dst: Varnode::unique(0x10, 1),
            space: SpaceId::Ram,
            addr: address.clone(),
        ...
        let mut arch = ArchSpec::new("differential-custom-space");
        arch.addr_size = 8;
        arch.set_memory_endianness(Endianness::Little);
        arch.add_space(AddressSpace::new(SpaceId::Custom(7), "custom7", 8));
        let artifact = SsaArtifact::raw(&[block], Some(&arch)).expect("custom-space artifact");
        let address = artifact.graph().insts[0].inputs[0];
        let mut state = DifferentialState::for_artifact(&artifact).expect("bound state");
        state.set_value(
            address,
            DifferentialBitVector::new(64, 0x20).expect("address"),
        );
        for (space, byte) in [
            (MachineAddressSpace::Ram, 0x11),
            (MachineAddressSpace::Custom(7), 0x22),
        ] {
            state.set_memory_byte(
                DifferentialMemoryLocation {
                    space,
        ...
        let report =
            check_block_differential(&artifact, 0x70fc, &state, DifferentialLimits::default());
        assert_eq!(
            report.conclusion(),
            DifferentialConclusion::NoMismatchObserved,
            "{report:?}"
        ...
        for run in [
            ...
            assert_eq!(run.memory_events.len(), 2);
            assert_eq!(run.memory_events[0].space, MachineAddressSpace::Ram);
            assert_eq!(run.memory_events[0].value.bits(), 0x11);
            assert_eq!(run.memory_events[1].space, MachineAddressSpace::Custom(7));
            assert_eq!(run.memory_events[1].value.bits(), 0x22);
            assert_eq!(run.final_memory.len(), 2);
            assert_ne!(run.final_memory[0].location, run.final_memory[1].location);
        }
    }

    #[test]
    fn runtime_aliasing_crosses_distinct_object_ids() {
        let mut block = R2ILBlock::new(0x70fd, 4);
        block.push(R2ILOp::Store {
            space: SpaceId::Ram,
            addr: Varnode::constant(0x40, 8),
        ...
        let mut arch = ArchSpec::new("differential-aliasing");
        arch.addr_size = 8;
        arch.set_memory_endianness(Endianness::Little);
        let artifact = SsaArtifact::raw(&[block], Some(&arch)).expect("aliasing artifact");
        let address = artifact.graph().insts[1].inputs[0];
        let mut state = DifferentialState::for_artifact(&artifact).expect("bound state");
        state.set_value(
            address,
            DifferentialBitVector::new(64, 0x40).expect("address"),
        );
        state.set_memory_byte(
            DifferentialMemoryLocation {
                space: MachineAddressSpace::Ram,
        ...
        let report =
            check_block_differential(&artifact, 0x70fd, &state, DifferentialLimits::default());
        assert_eq!(
            report.conclusion(),
            DifferentialConclusion::NoMismatchObserved,
            "{report:?}"
        ...
        for run in [
            ...
            assert_eq!(run.memory_events.len(), 2);
            assert_ne!(run.memory_events[0].object, run.memory_events[1].object);
            assert_eq!(
                run.memory_events[0].kind,
                DifferentialMemoryEventKind::Write
            );
            ...
            assert_eq!(run.memory_events[0].byte_address, 0x40);
            assert_eq!(run.memory_events[1].byte_address, 0x40);
            assert_eq!(run.memory_events[0].value.bits(), 0xaa);
            assert_eq!(run.memory_events[1].value.bits(), 0xaa);
            assert_eq!(run.final_memory[0].value, 0xaa);
        }
    }

    #[test]
    fn overlapping_accesses_are_source_ordered_and_endian_exact() {
        for (endianness, expected_read, expected_bytes) in [
            (Endianness::Little, 0x3040, [0x40, 0x30, 0x20, 0x10]),
            (Endianness::Big, 0x1020, [0x10, 0x20, 0x30, 0x40]),
        ] {
            let address = Varnode::register(0, 8);
            let mut block = R2ILBlock::new(0x70fe, 4);
            block.push(R2ILOp::Store {
                space: SpaceId::Ram,
                addr: address.clone(),
                val: Varnode::constant(0x1020_3040, 4),
            ...
            let mut arch = ArchSpec::new("differential-overlap");
            arch.addr_size = 8;
            arch.set_memory_endianness(endianness);
            let artifact = SsaArtifact::raw(&[block], Some(&arch)).expect("overlap artifact");
            let address = artifact.graph().insts[0].inputs[0];
            let mut state = DifferentialState::for_artifact(&artifact).expect("bound state");
            state.set_value(
                address,
                DifferentialBitVector::new(64, 0x80).expect("address"),
            );
            for offset in 0..4 {
                state.set_memory_byte(
                    DifferentialMemoryLocation {
                        space: MachineAddressSpace::Ram,
                        byte_address: 0x80 + offset,
                    },
                    0,
                );
            }
            let report =
                check_block_differential(&artifact, 0x70fe, &state, DifferentialLimits::default());
            assert_eq!(
                report.conclusion(),
                DifferentialConclusion::NoMismatchObserved,
                "{report:?}"
            ...
            for run in [
                ...
                assert_eq!(run.memory_events[1].kind, DifferentialMemoryEventKind::Read);
                assert_eq!(run.memory_events[0].object, run.memory_events[1].object);
                assert_eq!(run.memory_events[1].value.bits(), expected_read);
                assert_eq!(
                    run.final_memory
                        .iter()
                        .map(|byte| byte.value)
    ...
    #[test]
    fn word_addressed_memory_is_explicitly_incomplete() {
        let mut block = R2ILBlock::new(0x70ff, 4);
        block.push(R2ILOp::Load {
            dst: Varnode::unique(0x10, 1),
            space: SpaceId::Ram,
            addr: Varnode::register(0, 8),
        });
        let mut arch = ArchSpec::new("differential-word-addressed");
        arch.addr_size = 8;
        let mut ram = AddressSpace::ram(8);
        ram.word_size = 2;
        arch.add_space(ram);
        let artifact = SsaArtifact::raw(&[block], Some(&arch)).expect("word-addressed artifact");
        let address = artifact.graph().insts[0].inputs[0];
        let mut state = DifferentialState::for_artifact(&artifact).expect("bound state");
        state.set_value(
            address,
        ...
        let report =
            check_block_differential(&artifact, 0x70ff, &state, DifferentialLimits::default());
        assert_eq!(report.conclusion(), DifferentialConclusion::Incomplete);
        assert!(matches!(
            report.disposition(),
            DifferentialCaseDisposition::InterpreterUnsupported {
                side: DifferentialSide::Both,
                ..
            }
        ));
    ...
    #[test]
    fn little_and_big_endian_memory_match_with_ordered_events() {
        for endianness in [Endianness::Little, Endianness::Big] {
            let artifact = memory_artifact(endianness);
            let inputs = external_values(&artifact);
            let [address] = inputs.as_slice() else {
                panic!("one address input expected");
            };
            let mut state = DifferentialState::for_artifact(&artifact).expect("bound state");
            state.set_value(
                *address,
            ...
            let report =
                check_block_differential(&artifact, 0x7100, &state, DifferentialLimits::default());
            assert_eq!(
                report.conclusion(),
                DifferentialConclusion::NoMismatchObserved,
                "{report:?}"
            );
            let source = report.source().expect("source observation");
            assert_eq!(source.memory_events.len(), 2);
            assert!(matches!(
                source.memory_events[0].kind,
                DifferentialMemoryEventKind::Read
            ));
            assert!(matches!(
                source.memory_events[1].kind,
                DifferentialMemoryEventKind::Write
            ));
        }
    }
    ...
    #[test]
    fn residual_control_missing_memory_and_budget_never_match() {
        let mut branch = R2ILBlock::new(0x7200, 4);
        branch.push(R2ILOp::Branch {
            target: Varnode::ram(0x7210, 8),
        });
        let target = R2ILBlock::new(0x7210, 4);
        let artifact = SsaArtifact::raw(&[branch, target], None).expect("branch artifact");
        let report = check_block_differential(
            &artifact,
            0x7200,
            &DifferentialState::for_artifact(&artifact).expect("bound state"),
            DifferentialLimits::default(),
        );
        assert_eq!(report.conclusion(), DifferentialConclusion::Incomplete);

        let artifact = memory_artifact(Endianness::Little);
        let inputs = external_values(&artifact);
        let [address] = inputs.as_slice() else {
            panic!("one address input expected");
        };
        let absent = DifferentialState::for_artifact(&artifact).expect("bound state");
        let report =
            check_block_differential(&artifact, 0x7100, &absent, DifferentialLimits::default());
        assert_eq!(report.conclusion(), DifferentialConclusion::Incomplete);
        assert!(matches!(
            report.disposition(),
            DifferentialCaseDisposition::MissingBoundaryInput {
        ...
        let mut missing = DifferentialState::for_artifact(&artifact).expect("bound state");
        missing.set_value(
            *address,
            DifferentialBitVector::new(64, 0x20).expect("address"),
        );
        let report =
            check_block_differential(&artifact, 0x7100, &missing, DifferentialLimits::default());
        assert_eq!(report.conclusion(), DifferentialConclusion::Incomplete);
        assert!(matches!(
            report.disposition(),
            DifferentialCaseDisposition::MemoryOutOfDomain { .. }
    ...
    #[test]
    fn execution_failure_prefix_retains_prior_observable_write() {
        let mut block = R2ILBlock::new(0x7308, 4);
        block.push(R2ILOp::Store {
            space: SpaceId::Ram,
            addr: Varnode::constant(0x20, 8),
            val: Varnode::constant(0xaa, 1),
        ...
        block.push(R2ILOp::Load {
            dst: Varnode::unique(0x10, 1),
            space: SpaceId::Ram,
            addr: Varnode::constant(0x30, 8),
        });
        let mut arch = ArchSpec::new("differential-failure-prefix");
        arch.addr_size = 8;
        arch.set_memory_endianness(Endianness::Little);
        let artifact = SsaArtifact::raw(&[block], Some(&arch)).expect("prefix artifact");
        let mut state = DifferentialState::for_artifact(&artifact).expect("bound state");
        state.set_memory_byte(
            DifferentialMemoryLocation {
                space: MachineAddressSpace::Ram,
        ...
        let report =
            check_block_differential(&artifact, 0x7308, &state, DifferentialLimits::default());
        assert_eq!(report.conclusion(), DifferentialConclusion::Incomplete);
        assert!(matches!(
            report.disposition(),
            DifferentialCaseDisposition::MemoryOutOfDomain {
        ...
        for trace in [
            ...
            assert_eq!(trace.memory_events.len(), 1);
            assert_eq!(
                trace.memory_events[0].kind,
                DifferentialMemoryEventKind::Write
            );
            assert_eq!(trace.memory_events[0].value.bits(), 0xaa);
            assert_eq!(trace.final_memory[0].value, 0xaa);
        }
    }

    #[test]
    fn report_serialization_is_exact_ordered_and_deterministic() {
        let artifact = memory_artifact(Endianness::Little);
        let address = artifact.graph().insts[0].inputs[0];
        let make_state = |reverse: bool| {
            let mut state = DifferentialState::for_artifact(&artifact).expect("bound state");
            state.set_value(
                address,
                DifferentialBitVector::new(64, 0x20).expect("address"),
            );
            let mut bytes = vec![(0x20, 0x11), (0x21, 0x22), (0x22, 0x33), (0x23, 0x44)];
            if reverse {
                bytes.reverse();
            }
        ...
        let forward = make_state(false);
        let reverse = make_state(true);
        ...
        let first =
            check_block_differential(&artifact, 0x7100, &forward, DifferentialLimits::default());
        let second =
            check_block_differential(&artifact, 0x7100, &reverse, DifferentialLimits::default());
        assert_eq!(first, second);
        let json = serde_json::to_value(&first).expect("report JSON");
        assert_eq!(json["schema_version"], SEMANTIC_DIFFERENTIAL_SCHEMA_VERSION);
        assert_eq!(json["block_addr"], "0x0000000000007100");
        assert_eq!(json["admission"], "admitted");
        assert_eq!(json["conclusion"], "no_mismatch_observed");
        assert_eq!(json["disposition"]["kind"], "matched");
        assert_eq!(
            json["candidate_identity"]["evaluator_contract_version"],
            SEMANTIC_DIFFERENTIAL_EVALUATOR_CONTRACT_VERSION
        );
        assert!(
            json["candidate_identity"]["exact_candidate_postcard_hex"]
                .as_str()
        ...
        assert_eq!(
            ...
            json["initial_state"]["artifact_identity"]
        );
        assert_eq!(
            json["source"]["memory_events"][0]["byte_address"],
            "0x0000000000000020"
        );
        assert_eq!(
            json["source"]["memory_events"][0]["value"]["bits_hex"],
            "0x0000000044332211"
        );
        assert_eq!(
            json["source"]["memory_events"][0]["producer"]["block_addr_hex"],
            "0x0000000000007100"
        );
        assert_eq!(
            json["source"]["memory_events"][0]["producer"]["site"]["kind"],
            "op"
        );
        assert_eq!(
            json["source"]["memory_events"][0]["producer"]["site"]["ordinal_hex"],
            "0x0000000000000000"
        );
        assert_eq!(
