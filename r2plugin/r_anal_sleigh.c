...
extern int r2sleigh_interproc_helper_scope_budget_allows(size_t basic_block_count, unsigned int cost);
...
static bool function_exceeds_helper_scope_budget(const RAnalFunction *fcn) {
	...
	return r2sleigh_interproc_helper_scope_budget_allows ((size_t)bb_count, (unsigned int)cost) == 0;
}
...
static bool should_skip_decompile_symbolic_scope(const RAnalFunction *fcn) {
	return fcn && function_exceeds_helper_scope_budget (fcn);
}
	...
	if ((!strncmp (cmd, "sla.facts", 9) && (!cmd[9] || isspace ((unsigned char)cmd[9])))
		...
		fcn = (target_arg && *target_arg)
			...
			if (sleigh_session_input_init (&session_input, anal, ctx, fcn, &blocks, &typed_context,
						1,
						prefer_bounded_semantic_type_plan? 1: policy.type_interproc_max_iters,
